...
var InterfaceMaster = (function () {
    ...
    function createInstance(matchHandler) {
		...
		function interfaceObject(matchHandler){
			...
			var teamSelectMethod = "manual"; // Random or manual
			var partySize = 3;
			var battle;
			var playerRoster;
			var currentTeam = [];
			...
			this.importRandomizedRoster = function(pid, roster){
				multiSelectors[pid].setPokemonList(roster.slice(0, partySize));
			}
			...
			function startBattle(e){
				...
				var difficulty1 = $(".difficulty-select1 option:selected").val();
				var difficulty2 = $(".difficulty-select2 option:selected").val();
			...
			function randomizeTeam(e){
                ...
                if (playerId === 0) {
				var difficulty = $(".difficulty-select1 option:selected").val();
				} else {
				var difficulty = $(".difficulty-select2 option:selected").val();
				}
				var player = new Player(playerId, difficulty, battle);
				player.getAI().generateRoster(partySize, (roster) => self.importRandomizedRoster(playerId, roster));
			}
