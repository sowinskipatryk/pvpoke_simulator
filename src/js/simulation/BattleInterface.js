    ...
    function createInstance(matchHandler) {
		...
		function interfaceObject(matchHandler){
			...
			this.init = function(props, b, p){
				...
				$(".battle-window .button-stack").removeClass("active");
				...
				$(".battle-window .player-switch-sidebar").show();
				$(".battle-window .opponent-switch-sidebar").show();
				$(".player-switch-sidebar").removeClass("active");
				$(".opponent-switch-sidebar").removeClass("active");
				$(".switch-window").removeClass("active");
				$(".battle-window .pokemon-container .pokemon").removeClass('shake');
				$(".switch-window .pokemon, .player-switch-sidebar .pokemon").show();
				$(".switch-window .pokemon, .opponent-switch-sidebar .pokemon").show();
				$(".battle-window").attr("mode", props.mode);

				self.updateSwitchWindow(0);
				self.updateSwitchWindow(1);
			...
			this.update = function(response){
				...
				if(response.phase == "suspend_charged"){
					if(response.actor == 0){
						...
						response.phase = "suspend_charged_defense";
					}
				}
				...
				if(phase != response.phase){
					...
					switch(response.phase){
						case "suspend_charged_attack":
							$(".button-stack").removeClass('show');
							$(".battle-window .pokemon-container.self .pokemon").addClass('shake');
							$(".player-controls .move-labels .label").eq(response.move).addClass('charging');
							$(".player-switch-sidebar").removeClass("active");
							$(".opponent-switch-sidebar").removeClass("active");
                        ...
                        case "suspend_charged_defense":
						    $(".button-stack").removeClass('show');
							$(".battle-window .pokemon-container.opponent .pokemon").addClass('shake');
							$(".opponent-controls .move-labels .label").eq(response.move).addClass('charging');
							$(".player-switch-sidebar").removeClass("active");
							$(".opponent-switch-sidebar").removeClass("active");
						...
						case "animating":
							$(".battle-window .pokemon-container .pokemon").removeClass('shake');
                            $(".player-controls .move-labels .label").removeClass('charging');
                            $(".opponent-controls .move-labels .label").removeClass('charging');
							break;
				...
				var opponentSwitchOccurred = false;
				...
				for(var i = 0; i < response.pokemon.length; i++){
                    ...
                    var previousHealth = $poke.find(".hp-value .hp-now")[0].innerHTML;
                    var damage = pokemon.hp - parseInt(previousHealth);

					$poke.find(".hp-value .hp-now").html(pokemon.hp);
					$poke.find(".hp-value .hp-stat").html(pokemon.stats.hp);
					...
					if((activePokemon.length < i)||(activePokemon[i] != pokemon)){
						...
						if(response.turn == 1){
							...
							var pokemonIVRank = pokemon.getIVRank("overall").rank !== 0 ? "#" + pokemon.getIVRank("overall").rank : '??'
							$(".team-indicator").eq(i).find(".iv-info-box-rank").html("(" + pokemonIVRank + ")");
							$(".team-indicator").eq(i).find(".iv-info-box-stat").html(pokemon.ivs.atk + "-" + pokemon.ivs.def + "-" + pokemon.ivs.hp);
						}
                        ...
                        for(var n = 0; n < pokemon.chargedMoves.length; n++){
                            ...
                            if (i === 0) {
                                var $bar = $(".battle-window .player-controls .move-bar").eq(n);
                                $bar.show();
                                $(".player-controls .move-labels .label").eq(n).show();
                                ...
                                $(".battle-window .player-controls .move-labels .label").eq(n).html(chargedMove.name);
                            } else {
                                var $bar = $(".battle-window .opponent-controls .move-bar").eq(n);
                                $bar.show();
                                $(".opponent-controls .move-labels .label").eq(n).show();
                                $bar.find(".label").html(chargedMove.abbreviation);
                                $bar.find(".bar").attr("type",chargedMove.type);
                                $bar.find(".bar-back").attr("class","bar-back " + chargedMove.type);
                                $(".battle-window .opponent-controls .move-labels .label").eq(n).html(chargedMove.name);
                            }
                        }
						...
						if(i == 0){
						    ...
						    opponentSwitchOccurred = true;
						}
					}

					if ((damage < 0) && ((!playerSwitchOccurred && i === 0)||(!opponentSwitchOccurred && i === 1))) {
                        $poke.find(".hp-loss").html(damage);
                        $poke.find(".hp-loss").addClass('hp-show');

                        function timeoutClosure(index) {
                          setTimeout(function() {
                            $(".scene .pokemon-container .hp-loss").eq(index).removeClass('hp-show');
                          }, 450);
                        }
					...
					// Update move buttons
						...
						for(var n = 0; n < pokemon.chargedMoves.length; n++){
							...
							if(chargePercent >= 1){
								$bar.addClass("active");
								if (i == 0) {
								    $(".battle-window .player-controls .move-labels .label").eq(n).addClass("loaded");
								} else {
								    $(".battle-window .opponent-controls .move-labels .label").eq(n).addClass("loaded");
								}
							} else{
								$bar.removeClass("active");
								if (i == 0) {
								    $(".battle-window .player-controls .move-labels .label").eq(n).removeClass("loaded");
								} else {
								    $(".battle-window .opponent-controls .move-labels .label").eq(n).removeClass("loaded");
								}
							}
						}
				    }
				...
				for(var i = 0; i < response.players.length; i++){
					...
					if (i == 0) {
					    var switchSidebar = $(".battle-window .player-switch-sidebar");
					} else {
					    var switchSidebar = $(".battle-window .opponent-switch-sidebar");
					};

					var switchTimer = switchSidebar.find('.switch-timer');

                    if((player.getSwitchTimer() == 0)&&((phase == "animate")||(phase == "neutral"))){
                        switchSidebar.addClass("active");
                    } else{
                        switchTimer.html(Math.floor(player.getSwitchTimer() / 1000));
                    }

                    if(player.getSwitchTimer() == 0){
                        switchTimer.css("visibility","hidden");
                    } else {
                        switchTimer.css("visibility","visible");
                        switchSidebar.removeClass("active");
					}
				}
				...
				for(var i = 0; i < response.messages.length; i++){
					...
					if((message.index == 0)&&(message.str == "Blocked!")){
			...
			self.updateSwitchWindow = function(playerId){
				// Update the switch window Pokemon
				var team = players[playerId].getTeam();
				var index = 0;

				for(var i = 0; i < team.length; i++){
					if(team[i] != activePokemon[playerId]){
					    if (playerId === 0) {
						    var $targetContainers = [$(".player-switch-sidebar"), $(".switch-window")];
						} else {
						    var $targetContainers = [$(".opponent-switch-sidebar"), $(".switch-window")];
						}
			...
			self.completeSwitchAnimation = function(){
				...
				$(".battle-window .pokemon-container.animate-switch").each(function(index, value){
					...
					if( (timePassed >= parseInt($(this).attr("animation-duration")))&&(activePokemon[index].hp > 0)){
						...
						$(".team-indicator").eq(index).find(".iv-info-box-stat").html(activePokemon[index].ivs.atk + "-" + activePokemon[index].ivs.def + "-" + activePokemon[index].ivs.hp);
						var pokemonIVRank = activePokemon[index].getIVRank("overall").rank !== 0 ? "#" + activePokemon[index].getIVRank("overall").rank : '??'
						$(".team-indicator").eq(index).find(".iv-info-box-rank").html("(" + pokemonIVRank + ")");
			...
			function battleWindowClick(e){
				...
				if($(".battle-window .button-stack:hover").length > 0){

					// Pause or resume the game
					if($(".battle-window .button-stack .pause-btn:hover").length > 0){
						$(".battle-window .button-stack .pause-btn").toggleClass("active");
						battle.setPause($(".battle-window .button-stack .pause-btn").hasClass("active"));
					}

					// Open the restart confirm dialogue
					if($(".battle-window .button-stack .restart-btn:hover").length > 0){
						// Pause the game
						$(".battle-window .button-stack .pause-btn").addClass("active");
					...
					if($(".battle-window .button-stack .quit-btn:hover").length > 0){
						// Pause the game
						$(".battle-window .button-stack .pause-btn").addClass("active");
						battle.setPause(true);
				...
				if(($(".battle-window .player-controls .move-bar.active:hover").length == 0)&&($(".battle-window .switch-window:hover").length == 0)){
					// Fast move
					battle.queueAction(0, "fast", 0);
				} else{
					// Charged move
					...
					var index = $(".battle-window .player-controls .move-bar").index($target); // Getting the index of the move
